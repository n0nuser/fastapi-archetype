...
Generated from the [FastAPI Archetype](https://github.com/n0nuser/fastapi-archetype) cookiecutter template.

## Description

This service is built with FastAPI following Domain Driven Design (DDD) principles, best Docker practices, and a set of tools for development, testing, and deployment with the best code quality tools available.

## First Steps After Generation

1. `uv sync --all-groups`
2. Export the pinned runtime dependencies used by Docker:
   `uv export --format requirements.txt --no-dev --no-hashes --no-install-project --no-emit-workspace -o requirements.txt`
3. Copy the environment template: `cp src/.env.example src/.env` (then adjust values).
4. Replace this README with your own content.

## Table of Contents

- [{{ cookiecutter.project_name }}](#{{ cookiecutter.project_name | replace(' ', '-') | lower }})
